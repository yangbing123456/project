<template>
	...
	<!-- 项目里有很多循环的echarts   没有对接口嘛   基本就只差数据了  到时候你直接把数据 放到echarts配置里就行 -->
...
<script>
	...
	export default {
		...
		mounted() {
			this.getServerData()
			//账户信息
			loginApi.accountInformation().then(res => {
				console.log(res, '账户信息')
			})
			//账户金额信息
			loginApi.accountMoneyInformation().then(res => {
				console.log(res, '账户金额信息')
			})
		},
		methods: {
			//点击现金账户事件
