<template>
	<view class="container">
		<!-- 搜索 -->
		<view class="topbox">
			<u-search class="search" :placeholder="$t('time-select.time-select.5qzz6gcsask0')" v-model="keyword"
				action-text='' @search='search'>
			</u-search>
		</view>
		...
		<view class="bottom-btn">
			<u-button :hair-line="false" class="yes-btn"
				v-bind:class="{'class1':Boolean(value)}">{{$t('time-select.time-select.5qzz6gcsc8c0')}}</u-button>
		</view>
	</view>
</template>
	...
	export default {
		data() {
			return {
				keyword: '', //搜索值
				value: '', //单选按钮存放值
				//中间内容数组 这个数组目的是为了每次搜索之后重新给data数组进行赋值初始化  因为没有调接口  暂时这样把  后续调接口 可以把这个数组删掉
				dataArr: [{
						name: "Shanghai",
						id: 1,
						label: 'CMT +08:00'
